...
function render(userKey) {
  ...
  if (next.gate) {
    ...
    const mm = m % 60;
    meta.innerHTML = `
      <span class="countdown">倒计时 ${h>0?h+'h ':''}${mm}m 后 → ${next.gate.zhi}时 ${next.gate.gate}</span>
...
function renderTarotCard(card, question) {
  ...
  const elemIcon = { fire: '火', water: '水', air: '风', earth: '土', arcana: '主' }[elem] || '主';
  ...
  return `
    <div class="tarot-card elem-${elem}${card.reversed ? ' reversed' : ''}">
      ...
      <div class="t-emoji">${card.name}</div>
